    ...
    def tweak_diy(df):
        return (df
                ...
                .rename(columns={'年月': 'Date',
                                 ...
                                 'Others': 'その他'})
                .astype({'Date': 'string'})
                .assign(Date=lambda _df: (pd
                                          .to_datetime(_df['Date']
                                                       .str.replace('年', '/')
                                                       .str.replace('月', ''),
                                                       format='ISO8601')))
                .set_index(['Date']))
...
def evaluation_mul(item):
    ...
    pred_sales = (lr.intercept_
                  + lr.coef_[0][0]*diy['ＤＩＹ用具・素材'].loc['2022-12']
                  + lr.coef_[0][1]*diy['インテリア'].loc['2022-12']).to_numpy()
...
def evaluation(item):
    ...
    mae, mse, pred_sales, true_sales = predict_sales(net, device, X_true, y_true)
    return mae, mse, pred_sales, true_sales
...
def f(alpha):
    ...
    s12 = ((diy['ＤＩＹ用具・素材'].loc['2015-01':'2022-09'] - mx1)*(diy['インテリア'].loc['2015-01':'2022-09'] - mx2)).sum()
    syy = ((diy['商品販売額'].loc['2015-01':'2022-09'] - my)**2).sum()
    s1y = ((diy['ＤＩＹ用具・素材'].loc['2015-01':'2022-09'] - mx1)*(diy['商品販売額'].loc['2015-01':'2022-09'] - my)).sum()
    s2y = ((diy['インテリア'].loc['2015-01':'2022-09'] - mx2)*(diy['商品販売額'].loc['2015-01':'2022-09'] - my)).sum()
